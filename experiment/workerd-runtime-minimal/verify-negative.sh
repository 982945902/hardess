...
run_expect_failure_for_read_commands() {
  local label="$1"
  local expected_message="$2"
  shift 2

  run_expect_failure "print-resolved-model" "$label" "$expected_message" "$@"
  run_expect_failure "print-runtime-summary" "$label" "$expected_message" "$@"
}

run_expect_failure_for_all_commands() {
  local label="$1"
  local expected_message="$2"
  shift 2

  run_expect_failure "generate-config" "$label" "$expected_message" "$@"
  run_expect_failure_for_read_commands "$label" "$expected_message" "$@"
}
